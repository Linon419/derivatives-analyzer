...
### Critical: Distinguish Active vs Historical Divergences

When using divergence detection tools (e.g., `mcp__ccxt__detect-divergence`), **always verify whether the divergence is active or already released**.

| Status | Description | Action |
|--------|-------------|--------|
| **Active** | Divergence formed, price hasn't reacted yet | Report as current signal |
| **Releasing** | Price is moving in divergence direction | Report as "in progress" |
| **Released** | Price has fully reacted to divergence | Do NOT report as active signal |

### Verification Checklist

Before reporting a divergence signal:

1. **Check bar distance**: If `barDistance` is large (>20 bars), verify current price action
2. **Compare pivot prices to current price**:
   - For negative divergence: Has price already dropped significantly from the high?
   - For positive divergence: Has price already rallied significantly from the low?
3. **Cross-reference with user's TradingView**: Ask user what they see to validate

### Common Mistake

❌ **Wrong**: Reporting historical divergence as "bearish signal detected"
```
RSI negative divergence: $3,367 → $3,368 (29 bars ago)
→ This already triggered! Price fell from $3,369 to $2,866
```

✅ **Correct**: Clearly state the divergence status
```
RSI negative divergence (RELEASED): Formed 29 bars ago at $3,368 high
→ Signal has been fulfilled - price dropped 15% to $2,866
→ No active bearish divergence currently
```

### Reporting Template

When reporting divergences, use this format:

```
## Divergence Analysis

### Active Signals
- [indicator] [type] divergence forming at [price level]
- Status: Awaiting confirmation / Early stage

### Recently Released (Historical)
- [indicator] [type] divergence from [X bars ago]
- Trigger price: [price] → Result: [outcome]
- Status: ✅ Released / Fulfilled

### Current Technical State
Based on released divergences, the market has [absorbed the signal / completed the move]
```
